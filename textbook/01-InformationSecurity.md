...
The goals of a security team are centered around building controls that prevent, identify, and reduce (mitigate) the effects of risks partly realized by security defects and misconfigurations (vulnerabilities).  Such security vulnerabilities are identified and exploited by threats with malicious intent, such as but not limited to, stealing data or extorting victims.  Another goal of the security team is to limit the damaged caused by threats through establishing controls that detect and respond to malicious activities.
![[../images/01/a&c.png|Attacks and Countermeasures|400]]
The Attacks and Countermeasures box chart above displays the relationship between countermeasures, vulnerabilities, and threats.  It was inspired by a similar graphic and narrative prepared by Andrew Jaquith in his book "Security Metrics: Replacing Fear, Uncertainty, and Doubt".
## Threat Actors
...
### Reporting
Sharing the results of security risk management activities with organizational stakeholders helps to achieve security objectives by raising awareness of risks and advocating for resources.  The art and science of effective reporting starts with having an established reporting line, like the one that what was described in the "Roles and Responsibilities" section of this chapter.  Depending on the type of reporting, information may be gathered into spreadsheets, slide decks, and documents on a daily, weekly, monthly, quarterly and yearly basis.  The frequency of reporting is often commensurate with the operational and strategic need, where the former has a higher cadence and the latter is less frequent.

There is a plethora of items that can be reported.  A nonexhaustive list includes:
- Results of security audits and reviews
- Penetration test executive summaries
- Risk and control assessments
- Recent security incidents
- Metrics like key risk indicators (KRI) and key performance indicators (KPI)
- Program development and objectives and key results (OKRs)

> [!info] Info - Avoid Reporting Conflicts
>Particularly in smaller organizations, a security leader may often report into a non-security function.  For example, I have witnessed and heard of security teams reporting to the CTO, HR finance, or IT.  A reporting line established this way could introduce conflicts of interest for the non-security manager overseeing the security function as they may be inclined to omit security risk that could make their performance look poor to executive leadership or the board of directors.  This issue can be avoided by having the security function report directly to executives, risk committees, or the board of directors in order to provide an unbiased assessment of security risk.

Security management should be organized and implemented through **security programs**, which are groupings of controls focused on a risk category.  For instance, an organization that develops software should have an application security program that institutes controls like secure code training and static application security testing (SAST).  The volume, state, and maturity arc of these security programs would be reported to the executive team.  The saying, "if you are not measuring it, you are not managing it" holds true for security practitioners and managers.  Each control implemented in these programs can be quantifiably measured for performance and risk levels.  These numeric measurements, or **metrics**, are regularly collected into spreadsheets or databases.  Tracking metrics over time enables teams and managers to identify favorable and unfavorable trends and better prepares an organization to allocate resources where they are needed the most.  Example security metric tracking is demonstrated in the following table that includes common vulnerabilities and exposures (CVE) statistics by severity per year.
![[../images/01/cve_table.png|CVE by Year Table|240]]

The science of identifying quality and reliable data in a consistent manner will depend on the data type and source.  For instance, this data was sourced from the National Institute of Standards and Technology's (NIST) National Vulnerability Database (NVD) and other reputable sources.  Numerical metrics are well-suited for graphs and charts which transform abstract numbers into stories that are easily understood and concisely convey information - more of an art than science.  The tracked metrics within the "CVE by Year Table" above can be displayed as a stacked bar chart.  I've modified the default chart's colors, fonts, size, data labels and titles to make it more visually appealing.  I think that you would agree this chart instantly is more pleasurable to view, but also instantly conveys additional information not obvious in the table version.
![[../images/01/cve_chart.png|CVE by Year Chart|500]]
## Compliance
...
> 56. From the "Unattended Guest OS Install Setup" page, adjust the unattended install setup with your name as the username and a password of your choosing, set the "Domain Name" to lan, and check the "Guest Additions" option.  We will not be licensing Windows so do not worry about the Product Key; press the Next button.  Note, sometimes the installation fails due to the product key and some VirtualBox/Windows bug.  You might try entering all 0's for the product key value.  Then, during the setup within the running VM, you will need to select "I don't have a Product Key", and the installation should progress without further issue.

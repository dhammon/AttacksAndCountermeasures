...
## Incident Response
The discovery or detection of a breach in security should cause an immediate reaction to remediate the threat and recover systems known as **incident response**.  The *incident responder* is another security professional role held by an individual usually associated with the SOC team.  However, for smaller organizations, this role may not be held by someone within the organization which will instead rely on a third party to perform incident response services.  This is usually a good idea as most organizations to not have incident frequencies that would warrant having a full time incident responder available.  Similarly as SOC analysts and threat hunters use the SIEM to detect security breaches, the incident responder uses the system to perform their investigations.  They use the data within the SIEM to forensically identify where the initial breach occurred, the activity performed by the threat actor, and all the systems affected by the incident.

Incident response by its nature is alerted to incidents from the efforts of other roles.  Usually the SOC or threat hunting teams will validate a security incident and escalate the issue to an *incident response team (IRT)*.  While the graphic below demonstrates a delineation between the roles of SOC, threat hunters, and incident responders, the reality of who is responsible for the duties of these roles will vary depending on the organization.
...
The lifecycle of incident response efforts has two circular flow that progress states between distinct phases.  Not all organizations formally approach incident response this way; however, mature organizations put a lot of resources into being ready should a security incident occur.  Having a well defined lifecycle reduces the impact and duration of security incidents that contribute to an organization's financial standing.

A long cyclical arch between *preparation* and *post-incident recovery* phases strengthens an organization's incident response posture.  Being prepared enables the cyclical relationship *detection and analysis* as well as the *containment and eradication* phases.  These phases require an iterative approach that ensures the complete mitigation of an existing threat.  Upon doing so the incident response effort moves to the *post-incident recovery* phase.
![[../images/12/incident_phases.png|The Phases of Incident Response|600]]
Each phase in the lifecycle contains many processes and requires the coordination of many individuals and teams in an organization.  It can be very chaotic for an organization that suffers a security incident.  The **preparation** phase, which is continuous never ending, includes many activities that are conducted prior to an incident that will attempt to position the organization in the best position to handle the incident.  The more effort that is spent in this phase will assist an organization when responding to an incident.  During this phase an *incident response plan*, which will be covered in greater detail in the next section, is developed that defines or classifies security incidents among other things.  It is important that a communication strategy on the handling of security incident be agreed upon during the preparation phase which will ensure that members of the organization know who they can turn to for the status of the incident.  Other ideas developed during the preparation phase include how incidents will be tracked as well as the types of reports needed to communicate information surrounding the incident.  This phase will also designate who the members of the IRT are and their responsibilities.  It is beneficial for the organization to train and test their incident capabilities and feedback result of these efforts back into the incident response plan.  
...
Earlier in this chapter the development of the SIEM and its detections were introduced where I demonstrated how brute force attempts could be identified using the Windows Event 4625 (failed logon attempt) and Splunk's visualizations.  This type of information is critical during the **detection and analysis** phase of the incident response lifecycle.  Here SOC analysts and threat hunters will discover breaches of security from multiple attack vectors, indicators of compromise, and other sources.  Sometimes an alert originates from a user in the organization or a third party like a regulator, authority, customer, or security researcher.  Some sources and detections are more reliable providing stronger indications that security has been breached by a threat and should therefor be given higher priority when evaluating

>[!activity] Activity 12.6 - Splunk Enterprise Security
>In a previous activity within this chapter I demonstrated establishing a detection manually centered on a brute force attack on Windows.  We also explored the hundreds of tools, techniques, and procedures outlined in MITRE ATT&CK framework and found even more resources to build detection off of.  Manually creating SPLs, alerts, and dashboards to monitor all these threats would require a lot of resources which every organization caring for security would have to repeat the same level of effort which is entirely insufficient.  Therefore, Splunk, and many other vendors in this space, have developed predeveloped solutions that kickstart an organization's detections with preloaded SPLs, alerts and dashboards based on the most common threats called Splunk Enterprise Security.
>
>Enterprise Security offered by Splunk is a premium solution targeted to organizations designed for defense, prevention, and breach response.  Splunk users can install a trial version of the plugin package to experiment with before buying.  More information about the product can be found at https://www.splunk.com/en_us/products/enterprise-security.html.
>![[../images/12/enterprise_activity_homepage.png|Splunk Enterprise Security Homepage|600]]
>Because Splunk is the most widely adopted SIEM, and Enterprise Security is a very popular plugin to quickly enable organizations to introduce a SOC, readers are strongly encourage to take advantage of Splunk's free course "Introduction to Enterprise Security" (https://www.splunk.com/en_us/training/course-catalog.html?filters=filterGroup1FreeCourses&search=enterprise).
>![[../images/12/enterprise_activity_course.png|Splunk Course Catalog|600]]
>Registration to the course is free and walks the student through the use case, workflow, and interface of the tool.  The content is delivered in video format and concludes with an 11 question multiple choice quiz that can be taken unlimited times but requires a 75% to pass.  Afterwards you are awarded a certificate of completion which would look great on a resume!
>
